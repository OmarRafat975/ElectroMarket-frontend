...
export default function ProductCard({ product }) {
  ...
  let classCard = 'product sm:h-[30rem] p-3 mt-5 rounded mx-auto max-w-[20rem] ';

  return (
    <div className={classCard}>
      <Link to={`/product/${product._id}`}>
        <img src={product.images[0]} className="card-img-top w-fit object-fit" alt={product.name} />
        <div className="border-t h-[5rem] mb-5">
          <h3 className="text-blue-500 text-xl font-semibold mb-2">{product.name.length > 24 ? product.name.slice(0, 24) + '...' : product.name}</h3>
          <p className="text-gray-700">{product.description.length > 100 ? product.description.slice(0, 100) : product.description}...</p>
        </div>
      </Link>
